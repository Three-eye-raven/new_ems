...
void pvpcs_sofar_50kw_link_lose(const uint16_t deviceId)
{
...
void pvpcs_sofar_50kw_link_online(uint16_t deviceId, uint32_t timeStep) {}

void pvpcs_sofar_50kw_link_connect(const uint16_t deviceId)
{
        const uint32_t connectStatus = pvpcs_connect_status_get() | (1 << deviceId);
