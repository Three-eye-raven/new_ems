...
#include "pvpcs_sofar_50kw/pvpcs_sofar_50kw_data_protocol.h"

#include <stdio.h>
#include <unistd.h>
#include <pthread.h>

static ModbusMasterTcpChannel_T tcpChannel = {
        .config = {
                .ipv4 = "192.168.1.233",
                .port = 9000,
        },
        .channelInfo = {
                .channelName = "192.168.1.233:9000",
                .pCtx = NULL,
                .initialized = false,
                .slaveNum = 1,
                .slaveInfo = {
                        [0] = {
                                .deviceId = 0,
                                .slaveId = 0x01,
                                .pProtocol = &pvpcsSofar50kwDataProtocol,
                        },
                },
        },
};

void *pvpcs_tcp_communication_thread(void *arg)
{
    bool ret = modbus_master_tcp_channel_init(&tcpChannel);
    ModbusMasterChannelInfo_T *channle_info = &tcpChannel.channelInfo;
    if (!ret)
    {
        ...
        do
        {
            ...
            for(uint8_t device_id = 0; device_id < channle_info->slaveNum; device_id++)
            {
                ...
                //print_pvpcs_single_data(device_id);
                    modbus_master_slave_writable_infos_sync(channle_info, device_id);
            }
            ...
            if (pvpcs_connect_status_get() == 0) {
                //重新链接通道
                    modbus_master_channel_reconnect(channle_info);
            }
            usleep(100*1000);
        }while(1);
    }
...
static pthread_t pvpcs_tcp_communication_thread_id;


bool pvpcs_tcp_communication_thread_init(void)
{
    bool ret = true;
    if(pthread_create(&pvpcs_tcp_communication_thread_id, NULL, pvpcs_tcp_communication_thread, NULL) !=0)
    {
        ret = false;
    }
    return ret;
...
{
    pthread_join(pvpcs_tcp_communication_thread_id, NULL);
}
